...
const VIDEO_ROOT = path.join(DOCS, "data", "videos_noaa_japan_h2_limited_202609");
...
const ACTIVE_FROM = Date.parse("2026-09-07T00:00:00+09:00");
const ACTIVE_UNTIL = Date.parse("2026-09-14T00:00:00+09:00");
const SCHEDULE_JST = ["09:00", "14:00"];
const OUTPUT_NAME = "noaa_japan_horizontal_error.mp4";

function jstDayString(d = new Date()) {
  // Japan has no daylight-saving time.
  return new Date(d.getTime() + 9 * 60 * 60 * 1000)
    .toISOString().slice(0, 10);
}

function isWithinActiveWindow(d = new Date()) {
  const t = d.getTime();
  return Number.isFinite(t) && t >= ACTIVE_FROM && t < ACTIVE_UNTIL;
}

function jstRunInfo(d = new Date()) {
  const shifted = new Date(d.getTime() + 9 * 60 * 60 * 1000);
  const day = shifted.toISOString().slice(0, 10);
  const slotOverride = process.env.SWIFTTEC_VIDEO_SLOT || "";
  const slot = /^(0900|1400)$/.test(slotOverride)
    ? slotOverride
    : `manual-${shifted.toISOString().slice(11, 19).replaceAll(":", "")}`;
  return { day, slot, startedUtc: d.toISOString() };
}

function checkWindowCli(d = new Date()) {
  const active = isWithinActiveWindow(d);
  const message = active
    ? `Active: ${jstDayString(d)} JST`
    : "Expired or not yet active: no video will be generated.";
  console.log(message);
...
function pruneArchives(now = new Date()) {
  ensureDirs();
  const cutoff = jstDayString(now);
  for (const name of fs.readdirSync(ARCHIVE_DIR)) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(name)) continue;
    // KEEP_DAYS=1: retain today's JST folder, including both scheduled slots.
    if (name < cutoff) {
      fs.rmSync(path.join(ARCHIVE_DIR, name), {
        recursive: true, force: true,
      });
      console.log(`Pruned old JST archive: ${name}`);
    }
  }
}
    ...
    for (const movieType of MOVIE_TYPES) {
      ...
      for (const idx of indices) {
        if (!isWithinActiveWindow()) {
          throw new Error("Limited edition expired during frame capture.");
        }
      ...
      if (!isWithinActiveWindow()) {
        fs.rmSync(archiveOutput, { force: true });
        throw new Error("Limited edition expired during rendering; publication cancelled.");
      }
...
function buildIndex(results) {
  const days = fs.readdirSync(ARCHIVE_DIR)
    ...
    .map(day => {
      ...
      return {
        day_jst: day,
        slots: fs.readdirSync(dayDir)
          .filter(slot => fs.statSync(path.join(dayDir, slot)).isDirectory())
          .sort()
          .map(slot => {
            const file = path.join(dayDir, slot, OUTPUT_NAME);
            return {
              slot,
              file: fs.existsSync(file)
                ? path.relative(DOCS, file).replaceAll("\\", "/")
                : null,
            };
          }),
      };
  ...
  const doc = {
    version: "swifttec-v8.50-noaa-japan-h2-limited",
    updated_utc: new Date().toISOString(),
    active_from_jst: "2026-09-07 00:00",
    active_until_exclusive_jst: "2026-09-14 00:00",
    schedule_jst: SCHEDULE_JST,
    keep_days: KEEP_DAYS,
    visual: {
      viewport: VIDEO_VIEWPORT,
      heatmap_alpha: HEATMAP_STYLE.alpha,
      ...
      metric: "GPS HDOP × L1 horizontal error",
      source: "NOAA / archive_data_30m",
      camera: VIDEO_CAMERA.noaaJapanSameZoomSouth1,
    },
    latest: {
      noaa_japan_horizontal_error:
        "data/videos_noaa_japan_h2_limited_202609/latest/" + OUTPUT_NAME,
    },
    current_run: results,
    archive: days,
  };
...
async function main() {
  ...
  if (!isWithinActiveWindow(started)) {
    console.log("Limited edition expired: skipping without modifying outputs.");
    return;
  }

  ensureDirs();
  pruneArchives(started);

  const run = jstRunInfo(started);
  const archiveDayDir = path.join(ARCHIVE_DIR, run.day, run.slot);
  fs.mkdirSync(archiveDayDir, { recursive: true });

  const password = parsePagePassword();
  // Load Playwright only after the expiry gate. The lightweight gate job
  // does not need npm installation or a browser.
  ...
  // The run must also finish within the authorized period.
  if (!isWithinActiveWindow()) {
    console.log("Expiration reached during rendering; no new latest is published.");
    return;
  }
  ...
  console.log("Limited horizontal video generation complete:");

#!/usr/bin/env node
/**
 * Magnetic-storm scenario forecast video renderer.
 *
 * New standalone workflow:
 *   - ISEE Japan   : horizontal error only
...
const VIDEO_ROOT = path.join(DOCS, "data", "videos_storm_horizontal");
...
async function installStormScenario(page, source) {
  const result = await page.evaluate(({ anchors, sourceName }) => {
    function smoothstep01(x) {
      const t = Math.max(0, Math.min(1, Number(x) || 0));
      return 0.5 - 0.5 * Math.cos(Math.PI * t);
    }

    function interpKp(hours) {
      const h = Number(hours) || 0;
      if (!Array.isArray(anchors) || !anchors.length) return 5.0;
      if (h <= anchors[0].hour) return anchors[0].kp;

      for (let i = 0; i < anchors.length - 1; i++) {
        const a = anchors[i];
        const b = anchors[i + 1];
        if (h <= b.hour) {
          const frac = (h - a.hour) / Math.max(1e-9, (b.hour - a.hour));
          const u = smoothstep01(frac);
          return a.kp + (b.kp - a.kp) * u;
        }
      }
      return anchors[anchors.length - 1].kp;
    }

    function buildSyntheticSeries(startUtc, nSteps, dtMin) {
      const start = (startUtc instanceof Date) ? startUtc : new Date(startUtc);
      const out = [];
      for (let s = 0; s <= nSteps; s++) {
        const t = new Date(start.getTime() + s * dtMin * 60 * 1000);
        const hour = s * dtMin / 60;
        out.push({ t, kp: Number(interpKp(hour).toFixed(3)) });
      }
      return out;
    }

    const patched = function (_txt, startUtc, nSteps, dtMin, _mode = "auto") {
      return buildSyntheticSeries(startUtc, nSteps, dtMin);
    };

    try { window.__swiftStormKpAnchors = anchors; } catch {}
    try { window.__swiftBuildStormKpSeries = buildSyntheticSeries; } catch {}

    // 実処理で使われる関数を差し替え
    try { window.buildForecastKpSeriesFrom3DayText = patched; } catch {}
    try { buildForecastKpSeriesFrom3DayText = patched; } catch {}

    // 表示用メモ
    const memoLines = [
      "# Synthetic magnetic-storm scenario",
      "# KpF: 5 -> peak 8 -> 6",
      ...anchors.map(a => `+${String(a.hour).padStart(3, " ")}h : Kp ${a.kp.toFixed(1)}`)
    ].join("\n");

    const noaaText = document.getElementById("noaaKpText");
    if (noaaText) noaaText.value = memoLines;

    const manualText = document.getElementById("manualKpTableText");
    if (manualText) manualText.value = memoLines;

    // mode はどちらでも動くが、auto のままにしておく
    try { gKpInputMode = "auto"; } catch {}

    return {
      ok: true,
      source: sourceName,
      anchors,
      preview: [0, 12, 24, 36, 48, 60, 72, 84, 96].map(h => ({
        hour: h,
        kp: Number(interpKp(h).toFixed(2)),
      })),
    };
  }, {
    anchors: STORM_KP_ANCHORS,
    sourceName: source,
  });

  if (!result.ok) throw new Error(`storm scenario install failed for ${source}`);
  console.log(`${source}: synthetic storm Kp scenario installed`);
...
async function runForecast(page, source) {
  ...
  await page.waitForFunction(() => {
    ...
    const status2 = String(document.getElementById("v4ArchiveStatus")?.textContent || "");
    if (/予報失敗|失敗:|error/i.test(status) || /予報失敗|失敗:|error/i.test(status2)) {
      throw new Error(`${status} ${status2}`.trim());
    }
  ...
  const state = await page.evaluate(() => ({
    status:
      document.getElementById("swiftV52Status")?.textContent ||
      document.getElementById("v4ArchiveStatus")?.textContent ||
      "",
...
async function moveSliderAndStamp(page, index, target, movieType) {
  await page.evaluate(({ idx, sourceName, targetLabel, movieLabel }) => {
    ...
    if (stamp) {
      ...
      stamp.textContent =
        `${targetLabel} / ${movieLabel}\n${t}\nStorm scenario  KpF ${kpF} / KpB ${kpB}`;
    }
  }, {
    idx: index,
...
async function applyCamera(page, target) {
  const result = await page.evaluate(({ targetConfig, cameraConfig }) => {
    try {
      ...
      if (Array.isArray(camera.referenceBounds)) {
        ...
        return { ok: false, reason: `camera ${targetConfig.cameraKey} invalid` };
      }
...
async function renderOneTarget(browser, target, archiveDayDir, password) {
  ...
  try {
    ...
    await installStormScenario(page, source);
    ...
    for (const movieType of MOVIE_TYPES) {
      ...
      const tmp = fs.mkdtempSync(
        path.join(os.tmpdir(), `swifttec-storm-${target.key}-${movieType.key}-`)
      );
      ...
      outputs.push({
        ...
        thresholds_m: HEATMAP_STYLE.hdopTecLimits,
        colors: HEATMAP_STYLE.colors,
        kp_storm_anchors: STORM_KP_ANCHORS,
...
function buildIndex(results) {
  const fileFor = (day, prefix, suffix) => {
    ...
    return fs.existsSync(p)
      ? `data/videos_storm_horizontal/archive/${day}/${prefix}_${suffix}.mp4`
      : null;
  };
  ...
  const days = fs.readdirSync(ARCHIVE_DIR)
    ...
    .map(day => ({
      day_utc: day,
      isee_japan_horizontal_error: fileFor(day, "isee_japan", "horizontal_error"),
      noaa_japan_horizontal_error: fileFor(day, "noaa_japan", "horizontal_error"),
    }));

  const doc = {
    version: "swifttec-forecast-video-storm-horizontal-v1",
    ...
    visual: {
      ...
      isee_japan_camera: VIDEO_CAMERA.japan,
      noaa_japan_camera: VIDEO_CAMERA.noaaJapanSameZoomSouth1,
    },
    scenario: {
      kind: "synthetic_magnetic_storm",
      description: "KpF starts at 5, rises naturally to 8, then decays to 6",
      anchors: STORM_KP_ANCHORS,
    },
    latest: {
      isee_japan_horizontal_error: "data/videos_storm_horizontal/latest/isee_japan_horizontal_error.mp4",
      noaa_japan_horizontal_error: "data/videos_storm_horizontal/latest/noaa_japan_horizontal_error.mp4",
    },
    current_run: results,
    archive: days,
  };
...
async function main() {
  ...
  console.log("Storm horizontal video generation complete:");

...
TEC_ROOT = Path(os.environ.get("SWIFTTEC_TEC_ROOT", "docs/data/isee_tec"))
AI_ROOT = Path(os.environ.get("SWIFTTEC_AI_ROOT", "docs/data/ai/isee_japan"))
...
PAIR_TOLERANCE_MIN = int(os.environ.get("SWIFTTEC_KP_AI_PAIR_TOLERANCE_MIN", "10"))
...
def main() -> int:
    ...
    print(f"Loaded ISEE Japan TEC frame metadata: {len(metas_all)}")
    if len(metas_all) < 2:
        raise RuntimeError("Not enough ISEE Japan TEC archive frames. Run Fetch ISEE Japan High-Res TEC first.")
    ...
    coeff_doc = {
        "version": "swifttec-isee-japan-kp-model-rule-ai-region-v1",
        "updated_utc": iso(now),
        "data_source": "ISEE Japan High-Res TEC",
    ...
    grid_doc = {
        "version": "swifttec-isee-japan-kp-model-rule-ai-grid-v1",
        "updated_utc": iso(now),
        "data_source": "ISEE Japan High-Res TEC",
        "model_rule": "BaseTEC = PrevObservedTEC - F(KpB); ForecastTEC = BaseTEC + F(KpF)",
        "formula": "F(Kp) = k0 + k1*(Kp-3) + k2*(Kp-3)^2 + k3*(Kp-3)^3",
    ...
    perf_doc = {
        "version": "swifttec-isee-japan-kp-model-rule-ai-performance-v1",
        "updated_utc": iso(now),
    ...
    hist["version"] = "swifttec-isee-japan-kp-model-rule-ai-history-v1"
    ...
    print(
        f"ISEE Japan Kp model-rule AI training complete: pairs={len(pair_records)}, "

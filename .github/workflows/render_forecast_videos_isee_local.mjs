#!/usr/bin/env node
/**
 * SWIFT-TEC scheduled ISEE local close-up renderer.
 *
 * Runs at 06:00 and 12:00 JST.
 * Retention: one JST calendar day. Both runs are kept.
 *
 * Generates exactly four videos per run:
 *   - ISEE Hokkaido close-up: GPS HDOP × L1 horizontal error
 *   - ISEE Hokkaido close-up: GPS VDOP × L1 vertical error
 *   - ISEE Kyushu–Okinawa close-up: GPS HDOP × L1 horizontal error
 *   - ISEE Kyushu–Okinawa close-up: GPS VDOP × L1 vertical error
 *
 * Heatmap breakpoints: 3 / 5 / 10 / 20 m.
 */
...
const VIDEO_ROOT = path.join(DOCS, "data", "videos_isee_local");
...
// Keep the existing zoom level; only the camera windows are shifted.
const VIDEO_VIEWS = [
  // ISEE Hokkaido close-up
  {
    key: "hokkaido_close",
    sources: ["isee"],
    label: "Hokkaido Close",
    camera: {
      type: "fitBounds",
      // Hokkaido fills most of the map while keeping surrounding seas visible.
      bounds: [[40.5, 138.0], [46.5, 147.0]],
      padding: [10, 10],
    },
    thresholds: [3, 5, 10, 20],
  },

  // ISEE Kyushu–Okinawa close-up
  {
    key: "kyushu_okinawa_close",
    sources: ["isee"],
    label: "Kyushu + Okinawa Close",
    camera: {
      type: "fitBounds",
      // Kyushu at the top and Okinawa / Nansei Islands toward the bottom.
      bounds: [[23.5, 121.5], [34.5, 135.5]],
      padding: [10, 10],
    },
    thresholds: [3, 5, 10, 20],
  },
...
function currentJstRunSlot(d = new Date()) {
  ...
  if (Math.abs(total - 6 * 60) <= 30) return { day: p.day, slot: "0600" };
  if (Math.abs(total - 12 * 60) <= 30) return { day: p.day, slot: "1200" };
...
async function renderOneSource(browser, source, archiveDayDir, password) {
  ...
  try {
    ...
    // GPS-only constellation for both HDOP×L1 and VDOP×L1 movies.
  ...
  if (fs.existsSync(dayDir)) {
    for (const slot of fs.readdirSync(dayDir).sort()) {
      ...
      const files = fs.readdirSync(slotDir)
        ...
        .sort()
        .map(name =>
          `data/videos_isee_local/archive/${run.day}/${slot}/${name}`
        );
    ...
    for (const source of view.sources) {
      for (const movieType of MOVIE_TYPES) {
        ...
        latest[`${source}_${view.key}_${movieType.key}`] =
          `data/videos_isee_local/latest/${fileName}`;
      }
    }
  }

  const doc = {
    version: "swifttec-forecast-video-v1-isee-local",
    updated_utc: new Date().toISOString(),
    schedule_jst: ["06:00", "12:00"],
    ...
    visual: {
      ...
      colors: HEATMAP_STYLE.colors,
      gps_constellation_only_for_dop_error: true,
